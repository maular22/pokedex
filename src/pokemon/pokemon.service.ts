...
export class PokemonService {
  ...
  async findOne(term: string) {
    ...
    if (!pokemon)
      throw new NotFoundException(
        `Pokemon whith id, name or no "${term}" not found`,
      );
  ...
  async remove(id: string) {
    ...
    if (deletedCount === 0) {
      throw new BadRequestException(` Pokemon whith id "${id}" npt found`);
    }
    return { ok: true };
  }
  ...
  private handleExeceptions(error: any) {
    if (error.code === 11000) {
      throw new BadRequestException(
        `Pokemosn exists in db ${JSON.stringify(error.keyValue)}`,
      );
    }
    console.log(error);
